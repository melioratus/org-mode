...
[fn:2] [[https://en.wikipedia.org/wiki/Literate_programming][https://en.wikipedia.org/wiki/Literate_programming]]

[fn:3] [[http://kitchingroup.cheme.cmu.edu/blog/2014/03/27/Literate-programming-in-python-with-org-mode-and-noweb/][http://kitchingroup.cheme.cmu.edu/blog/2014/03/27/Literate-programming-in-python-with-org-mode-and-noweb/]]

[fn:4] [[https://github.com/fniessen/refcard-org-mode][https://github.com/fniessen/refcard-org-mode]]

[fn:5] [[https://github.com/fniessen/org-html-themes][https://github.com/fniessen/org-html-themes]]

[fn:6] [[http://howardism.org/Technical/Emacs/literate-devops.html][http://howardism.org/Technical/Emacs/literate-devops.html]]
...
#+BEGIN_SRC js :cmd "node" :var data=(org-sbe headline-test1) :results replace output :var pattern="^([*]+)( +(TODO|DONE)?( +(\\[#[a-zA-Z]\\]))?( +(.+))?( +(([:\\w@#%]+))+))?" :wrap "SRC js :eval never"
...
#+BEGIN_SRC js :cmd "node" :var data="**** other stuff goes here" :results replace output :var pattern=STARS() :wrap "SRC js :eval never" :exports results 
  var re = new RegExp(pattern);
  console.log(data.match(re))
#+END_SRC
...
#+RESULTS:
#+BEGIN_SRC js :eval never
[ '**** ', '****', index: 0, input: '**** other stuff goes here' ]
#+END_SRC
...
#+BEGIN_SRC js :cmd "node" :var data="**** DONE stuff goes here" :results replace output :var pattern=(concat (org-sbe STARS) (org-sbe KEYWORD)) :wrap "SRC js :eval never" :exports results 
  var re = new RegExp(pattern);
  console.log(data.match(re))
#+END_SRC
...
#+BEGIN_SRC js :cmd "node" :var data="**** DONE [#A] stuff goes here" :results replace output :var pattern=(concat (org-sbe STARS) (org-sbe KEYWORD) (org-sbe PRIORITY)) :wrap "SRC js :eval never" :exports results 
  var re = new RegExp(pattern);
  console.log(data.match(re))
#+END_SRC
...
#+BEGIN_SRC js :cmd "node" :var data="**** DONE [#A] *this* /is/ _the_ =title= +of+ ~my~ headline :notattag:" :results replace output :var pattern=(concat (org-sbe STARS) (org-sbe KEYWORD) (org-sbe PRIORITY) (org-sbe TITLE)) :wrap "SRC js :eval never" :exports results 
  var re = new RegExp(pattern);
  console.log(data.match(re))
#+END_SRC
...
#+RESULTS:
#+BEGIN_SRC js :eval never
[ '**** DONE [#A] *this* /is/ _the_ =title= +of+ ~my~ headline :notattag:',
  '****',
  'DONE',
  'DONE',
  ' [#A]',
  ...
  '*this* /is/ _the_ =title= +of+ ~my~ headline :notattag:',
  index: 0,
  input: '**** DONE [#A] *this* /is/ _the_ =title= +of+ ~my~ headline :notattag:' ]
#+END_SRC
...
#+BEGIN_SRC js :cmd "node" :var data="**** DONE [#A] *this* /is/ _the_ =title= +of+ ~my~ headline :notattag: :tag1:@tag2:#tag3:%TAG4#:" :results replace output :var pattern="^(?:[*]+) (?:TODO|DONE)? (?:\\[#[a-zA-Z]\\])? (?:.+)? (([:\\w@#%]+))+"
  var re = new RegExp(pattern);
  console.log(data.match(re))
#+END_SRC
