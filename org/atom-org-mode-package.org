...
#+CALL: elisp/display-this-here(data=org-syntax-priority-definition)

#+RESULTS:
:   PRIORITY is a priority cookie, i.e. a single letter preceded by
:   a hash sign # and enclosed within square brackets.

#+BEGIN_SRC js :cmd "node" :var data="**** DONE [#A] stuff goes here" :results replace output :var pattern="^(?:[*]+) (?:TODO|DONE)? (\\[#[a-zA-Z]\\])"
  var re = new RegExp(pattern);
  console.log(data.match(re))
#+END_SRC
...
#+BEGIN_SRC js :cmd "node" :var data="**** DONE [#A] *this* /is/ _the_ =title= +of+ ~my~ headline :notattag:" :results replace output :var pattern="^(?:[*]+) (?:TODO|DONE)? (?:\\[#[a-zA-Z]\\])? (.+)"
  var re = new RegExp(pattern);
  console.log(data.match(re))
#+END_SRC
