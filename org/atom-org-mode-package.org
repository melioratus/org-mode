...
#+BEGIN_SRC ruby :var object_link_type="radio-link" :var object_link_pattern=radio-link-js 
      ...
      'patterns':[
          {
            ...
            'captures':
              ...
              '2':
                'name': 'markup.underline.link.radio-link.org-mode'
              '3':
...
********* Implementation notes about how ~PROTOCOL~ regex diverges from ~org-link-types-re~ default value.
  ...
  - Updates to allow expression to match most[fn:11] the [[file:atom-org-mode-package.org::*External Link Patterns][External Link Patterns]].
    ...
    - ~\\)~ \rightarrow ~)~
    - ~\\|~ \rightarrow ~|~
    - ~PROTOCOL~ \rightarrow ~(PROTOCOL|[.]?/)~ to allow matching plain links without ~PROTOCOL~ to imitate observed behavior in emacs.
